...
function preload() {
  ...
  character.push(new Character(300, 600, 80, 80, 'Assets/sprite.png', animations));
...
function draw() {
  switch(game.state) {
    ...
    if (millis() - lastDanceMoveTime >= danceMoveInterval) {
     ...
     danceMoveInterval -= intervalReductionRate * (game.elapsedTime / 5);
      // Ensure the interval doesn't go below a minimum value
      danceMoveInterval = max(danceMoveInterval, 500); // Minimum interval: 0.5 seconds
    }
